...
TRANSLATIONS += $$files($$PWD/quiterss_*.ts)
...
!isEmpty(TRANSLATIONS) {
  ...
  updateqm.output = $$BUILD_DIR/lang/${QMAKE_FILE_IN_BASE}.qm
  ...
  for(tsfile, TRANSLATIONS) {
    ...
    qmfile ~= s/\.ts$/.qm/
    LANG_QM_FILES += $$BUILD_DIR/lang/$$qmfile
...
unix:!mac {
  languages.path = $$DATA_DIR/lang
  INSTALLS += languages
}
win32 {
  languages.path = $$DESTDIR/lang
  INSTALLS += languages
}
mac {
  languages.path = Contents/Resources/lang
